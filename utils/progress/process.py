...
def create_torrent_if_needed(file_dir, torrent_dir):
    ...
    if result:
        ...
        tmdb_id, imdb_id, media_type, child, keywords ,chinese_name,item_type = handle_media(chinese_title, english_title, year, season, media, maker)
        if not media_type:
        ...
        if not existing_torrent_path:
            ...
            return torrent_path, chinese_title, english_title, year, season, media, codec, audiocodec, maker, tmdb_id, imdb_id, media_type, child, keywords, upload_title ,chinese_name,item_type
        else:
            logger.info(f"找到同名种子：{existing_torrent_path}，跳过制种")
            return existing_torrent_path, chinese_title, english_title, year, season, media, codec, audiocodec, maker, tmdb_id, imdb_id, media_type, child, keywords, upload_title ,chinese_name,item_type
    else:
...
#mediainfo/bdinfo解析
def process_media_directory(torrent_path, file_dir, pic_num, username, password, chinese_title, english_title, year, season, media, codec, audiocodec, maker, tmdb_id, imdb_id, media_type,  child, internal, personal, keywords, upload_title, chinese_name, item_type):
    ...
    if file_name:
        ...
        if action == "mediainfo":
            ...
            sys.exit()
        kimoji_upload(torrent_path, file_name, username, password, chinese_title, english_title, year, season, media, codec, audiocodec, maker ,pic_urls, tmdb_id, imdb_id, media_type, child, resolution, bd_info, internal,personal,keywords,upload_title, mediainfo_output,chinese_name,item_type)
    else:
...
def handle_media(chinese_title, english_title, year, season, media, maker):
    media_type, tmdb_id, item_type, chinese_name, child, keywords = search_tmdb(english_title)
    imdb_id = None
    if tmdb_id != 0:
        imdb_id = get_additional_ids(tmdb_id, item_type)
    else:
        imdb_search_id, media_type = search_imdb(english_title, media)
        if imdb_search_id:
            tmdb_id, imdb_id = search_ids_from_imdb(imdb_search_id, english_title, media_type)
    print(f'最终结果:\nmedia_type:{media_type}\ntmdb:{tmdb_id}\nimdb:{imdb_id}')
    return tmdb_id, imdb_id, media_type, child, keywords ,chinese_name,item_type
...
#def handle_found_media(tmdb_id, item_type, media_type, chinese_title, english_title, child, year, season, maker):
    #imdb_id = get_additional_ids(tmdb_id, item_type, english_title)
